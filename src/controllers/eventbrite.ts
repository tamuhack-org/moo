...
class EventbriteController {
  ...
  async fetchEvents(): Promise<EventbriteEvent[]> {
    if (!this.token) {
      throw new Error(
          'The EVENTBRITE_TOKEN environment variable is not set. Please set it.');
    }
    const tamuhackEventsUrl = `${
        EVENTBRITEAPI_URL}/users/me/events/?time_filter=current_future&token=${
        this.token}`;
  ...
  private async fetchEventData(eventId: string): Promise<EventbriteEvent> {
    const individualEventUrl =
        `${EVENTBRITEAPI_URL}/events/${eventId}/?token=${this.token}`;
  ...
  private async fetchEventVenueData(venueId: string): Promise<string> {
    ...
    const individualEventVenueUrl =
        `${EVENTBRITEAPI_URL}/venues/${venueId}/?token=${this.token}`;
...
const eventbriteController =
    new EventbriteController(process.env.EVENTBRITE_TOKEN);
